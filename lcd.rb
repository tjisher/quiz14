...
## LCDStatic Module
# static and support for LCD Class
# provides no exception protection, conversion helpers or storage
# converts characters to simplified 5 layer grid, scaleable
#
# case sensative
# not implemented characters are ignored, completely invalid string convertable /
# will return empty string
#
# default method overides:
# none
#
# key methods:
# create_output
...
module LCDStatic
	...
	#
	# parameters:
	# values, string to be displayed
	#
	# optional parameters:
	# scale, otherwise default 2
	# logger, otherwise records nothing
	# character_encoding, otherwise default character set ( \s, :, 0-9)
	# horizontal_character, otherwise default "-"
	# vertical_character, otherwise default "|"
	# special_character, otherwise default "."
	def create_output(values, scale=SCALE_DEFAULT, logger=nil, 
		...
		logger << { values: values, scale: scale, finished_at: Time.now, 
			output: output } if logger
		output
	end
...
#
# default method overides:
# to_s, time object specificly ouputs in HH:MM format
#
# key methods:
...
class LCD 
	...
	#adds to logger
	def update_output
		@output = self.create_output( @values, @scale, @logger, 
			@character_encoding, @horizontal_character, @vertical_character)
	end
